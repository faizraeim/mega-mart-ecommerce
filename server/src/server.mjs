...
app.get('/products', (req, res) => {
  res.send(products); // Send the JSON data as a response
});

// server port number comming from .env file or fallback
...
// user imported file to json format
...
// api response sending all products as json
app.get("/api/products", (request, response) => {
  console.log(request.query)
  const { query: { filter, value } } = request;

  if (!filter && !value) return response.send(products)
  if (filter && value) return response.send(products.filter((user) => user[filter].includes(value)))
...
// api response using params to filter by ID
app.get("/api/products/:id", (request, response) => {
  console.log(request.params)
  const parsedID = parseInt(request.params.id)
  console.log(parsedID)
  if (isNaN(parsedID)) return response.sendStatus(400).send({ message: "Bad Request. Invalid ID" });
  const findProduct = (products.products).find((product) => product.id === parsedID)
