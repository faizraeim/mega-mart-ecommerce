...
function Categories() {
    ...
    return (
        <>
            {loading && <div className="flex space-x-4 max-w-7xl mx-auto border-y border-border py-6">Loading...</div>}
            {error && <div className="flex space-x-4 max-w-7xl mx-auto border-y border-border py-6">Error: {error}</div>}

            {!loading && !error && (
                <div className="max-w-7xl mx-auto px-4 pt-8 pb-4 border-b border-border">
                    <div className="flex items-center justify-between  mb-4">
                        <div>
                            <p className="text-primary text-sm font-semibold uppercase tracking-wide">Browse categories</p>
                            <p className="text-heading text-xl font-bold">Find what you need faster</p>
                        </div>
                        <span className="text-xs text-light">Showing {categories.length} categories</span>
                    </div>
